...
const ExpensesView: React.FC<ExpensesViewProps> = ({ user, appSettings, onAdd, onEdit, onBack }) => {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden" style={{ background: '#0b0e1a' }}>
      {showExportModal && (
        <ExportFormatModal onSelect={handleExportFormat} onClose={() => setShowExportModal(false)} />
      )}

      {/* STICKY HEADER */}
      <div className="sticky top-0 z-30 px-4 pb-2 md:px-6 flex-shrink-0" style={{ paddingTop: 'max(16px, calc(env(safe-area-inset-top, 0px) + 8px))', background: 'rgba(11,14,26,0.92)', backdropFilter: 'blur(20px)' }}>
